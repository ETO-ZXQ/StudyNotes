...
| 相对精度 | 1m:±3mm |
| 深度处理芯片 | MX6000 |
| 深度图分辨率 | 1280x1024@7/10/15/30fps<br>640x480@7/10/15/30fps<br>320x240@7/10/15/30fps<br>160x120@7/10/15/30fps |
| 深度FOV | 远距：H57.6° ±1° V45.5° ±1° D69° ±1° @ 1m<br>近距：H56.6° ±1° V45.5° ±1° D68° ±1° @ 1m |
| 彩色图分辨率 | 1920x1080@30FPS<br>1280x720@30FPS<br>800x600@30FPS<br>640x480@30FPS |
| 彩色FOV | 16 : 9：H69.7° ±1° V42.8° ±1° D77.2° ±1°<br>4 : 3：H67.8 ±1° V53.6° ±1° D80°±1° |
| UVC | 支持 |
| 数据传输 | USB3.0 Type-C |
| 供电 | 接口：USB 3.0<br>平均工作电流/功率：700mA/3500mW<br>峰值工作电流/功率：760mA/3800mW<br>（ Deptd VGA @ 30fps，RGB 1080P @ 30fps ， 25℃）<br>*最大电流不超过850mA，电源供电能力建议达到5V1.0A及以上* |
| 风扇主动散热 | 支持（支持软件开关）|
| 工作温度 | 0°C-40°C |
| 适用场景 | 室内 |
| 安全性 | Class1 激光 |
| 支持操作系统 | Android / Linux / Windows 7 & 10 |
| 整体尺寸 | 178.32 x 35.32 x 38.10 mm |

### > 官网资料

相机SDK开发教程：[3D视觉开发者社区|3D视觉开发者社区SDK功能说明以及使用步骤(New)](https://developer.orbbec.com.cn/v/forum_detail/14557)

相机SDK下载：[3D视觉AI开放平台|开发者资源中心](https://vcp.developer.orbbec.com.cn/resourceCenter?defaultSelectedKeys=55)

Orbbec SDK Python Wrapper文档：[3D视觉AI开放平台|Orbbec SDK for Python 使用手册](https://vcp.developer.orbbec.com.cn/documentation?doc=doc-70)

OrbbecViewer工具手册:[3D视觉AI开放平台|OrbbecViewer工具手册](https://vcp.developer.orbbec.com.cn/documentation?doc=doc-65)
...
- 深度相机数据格式`OBFormat.Y16`：返回数组每个值为一个16位无符号整数`uint16`，单位为mm，不在工作范围内为0。例如：2600，即该像素点对应的物理空间中该点距相机光心2.6m

##### 取数据流程

1. 向`pyorbbecsdk.Pipeline`对象请求一帧复合帧。

    ```Python
    # 以阻塞的方式等待一帧数据，该帧是一个复合帧，里面包含配置里启用的所有流的帧数据，并设置帧的等待超时时间为100ms。
    frames: pyorbbecsdk.FrameSet = pipeline.wait_for_frames(100)
    ```
...
3. 各自流的帧调用`.get_data()`方法并转换成`numpy`的数组。注意：这步得到的是`numpy`类型的 **一维数组**
...
**注意：** 取数据时需要检查复合帧`pyorbbecsdk.FrameSet`与各自流的帧`pyorbbecsdk.XXXFrame`是否为`None`。为`None`则需要重复取数据。由于相机原因，数据会经常为`None`，并会输出一些警告or错误信息，重复取数据就行，警告or错误信息不必理会，不会影响后续程序。

##### D2C（深度图对齐彩色图）

- **对齐**：由于3D相机不同摄像头间有距离，所以拍摄得到的画面是不同相机位姿下的，需要统一。D2C即将深度图对齐彩色图，使相同像素坐标的像素对应的是物理空间中同一点。注意：D2C后，深度图四周会有一圈较宽的无效区域，其值为0。
- **设置**
...
import cv2
import pyorbbecsdk
from pyorbbecsdk import OBFormat, OBSensorType, OBPropertyID, OBAlignMode, VideoStreamProfile, OBCameraIntrinsic


# 打印相机SDK版本
def print_SDKversion():
    SDKversion = pyorbbecsdk.get_version()
    print(SDKversion)
...
class AstraPlus():
    def __init__(self, model=0):
        ...
        # 创建流对象
        self.pipeline = pyorbbecsdk.Pipeline(self.device)    # 定义流程pipeline
        self.config = pyorbbecsdk.Config()                   # 定义配置config
        ...
        if self.model == 0:     # 只开启彩色相机
            ...
            print("AstraPlus Error: model is not 0 or 1")
            return
    ...
    def get_color_depth_images(self):
        while True:
            ...
            color_frame = frames.get_color_frame()  # OBFormat.RGB
            depth_frame = frames.get_depth_frame()  # OBFormat.Y16
            # 彩色图
            if color_frame is None:
                continue
            # 深度图
        ...
        # 转换为8位深度图
        depth_image_8bit = cv2.convertScaleAbs(depth_image, alpha=0.1)  # 数值缩小10倍，单位变为cm，丢掉一位精度。超过255（即2.55m）置255
    ...
    def get_CameraParam(self) -> OBCameraIntrinsic:
        ...
        color_intrin = self.param.rgb_intrinsic     # <class 'pyorbbecsdk.OBCameraIntrinsic'>

        depth_intrin = self.param.depth_intrinsic   # <class 'pyorbbecsdk.OBCameraIntrinsic'>

        # 例：< OBCameraIntrinsic fx = 475.757019 fy = 475.757019 cx = 330.705933 cy = 239.390900 width = 640 height = 480 >
    ...
    # 打印相机支持设置的属性
    def print_CameraSupportedProperty(self):
        support_property_count = self.device.get_support_property_count()
        for i in range(0, support_property_count):
            PropertyItem: pyorbbecsdk.OBPropertyItem = self.device.get_supported_property(i)

            Property_name: str = PropertyItem.name

            Property_ID: pyorbbecsdk.OBPropertyID = PropertyItem.id
            Property_ID_name = Property_ID.name
            Property_ID_value = Property_ID.value

            Property_permission: pyorbbecsdk.OBPermissionType = PropertyItem.permission
            Property_permission_name = Property_permission.name
            Property_permission_value = Property_permission.value

            Property_type: pyorbbecsdk.OBPropertyType = PropertyItem.type
            Property_type_name = Property_type.name
            Property_type_value = Property_type.value

            print(i, '\t', Property_name, '\t', Property_ID_name, Property_ID_value, '\t', Property_permission_name, Property_permission_value, '\t', Property_type_name, Property_type_value)
...
if __name__ == "__main__":
    ...
    while True:
        color_image, depth_image, depth_image_8bit = astra.get_color_depth_images()
        cv2.imshow("color_image", color_image)
        cv2.imshow("depth_image_8bit", depth_image_8bit)

        key = cv2.waitKey(1)
        if key == ord('q') or key == 27:    # ESC_KEY = 27
            break
